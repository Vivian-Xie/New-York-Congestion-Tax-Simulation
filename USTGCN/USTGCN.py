...
class DataCenter(object):
        ...
        for idx in range(st_day, en_day + 1 - day, 1):
            ...
            for st in range(st_point, en_point):
                ...
                for it in range(st, st + 12):
                    ...
                    a_data.append(a) #一个时间戳

                a_data = np.concatenate(a_data, axis=0) #12个时间戳，组成一组预测数据（12，228，8）
    ...
    def load_my_data(self,ds,hr_sample,day,pred_len,
                  the_day,):
        ...
        ts_data = []
        st_point = the_day*timestamp
        en_point = (the_day + 1) * timestamp
        for st in range(st_point, en_point):#从当天的第一个时间戳开始循环
            a_data = []
            ...
            for it in range(st,st+12):
                ...
                a_data.append(a)
            a_data = np.concatenate(a_data, axis=0)  # 12个时间戳，组成一组预测数据（12，228，8）
            ts_data.append(a_data) # 最终得到（288,12，228，8）
...
def inference(raw_features, USTGCN, regression,):
...
class DataLoader:
    ...
    def __init__(self, config,ds,pred_len,the_day):
    ...
    def load_predict_data(self):
        print("Loading Data...")
        test_data = self.dataCenter.load_my_data(self.ds, self.hr_sample, self.day,
                                                          self.pred_len,self.the_day)
        adj = self.dataCenter.load_adj(self.ds)
        print("Data Loaded")
        print("Dataset: ", self.ds)
        print("Total Nodes: ", adj.shape[0])
        print("day for predict: ", self.the_day)
        return test_data,  adj
...
class TrafficModel:
    ...
    def __init__(self, train_data,train_label,test_data,test_label,adj, 
      ...
      if self.train_data is not None:
          ...
          self.test_data = torch.FloatTensor(self.test_data).to(device)
          self.new_data = self.fake_data(self.test_data)
          self.new_data = torch.FloatTensor(self.new_data).to(device)
    ...
    def fake_data(self,test_data):
        # test_data -> (288,12，228，8)
        test_data = test_data.cpu().numpy().copy()
        data_ori = test_data.copy()
        ...
        (x3, x4) = (180, 220)
        # # mask
        # mask_1 = (x1 >= test_data) & (x3 >= test_data >= x2) & (test_data >= x4)
        # mask_2 = (x2 >= test_data >= x1) & (x4 >= test_data >= x3)

        # 沿着第一个维度（288）进行平滑衰减
        for i in range(test_data.shape[0]):
            test_data[i] = test_data[i] * decay_factor1

        for i in range(x1, x2):
            test_data[i] = test_data[i] * decay_factor2

        for i in range(x3, x4):
            test_data[i] = test_data[i] * decay_factor2

        # 对y进行平滑处理
        test_data = gaussian_filter1d(test_data, sigma=5)

        # plt.plot(data_ori[0,0,:,0], label='sin(x)')
        # plt.plot(y_smooth[0,0,:,0], label='cos(x)')
        # plt.show()
    ...
    def run_predict(self,   ):
        ...
        pred = []
        pred_new = []
        tot_instance = len(self.test_data)-11
        idx = np.array([x for x in range(tot_instance)])


        for data_timestamp in idx:
            # test_label
            raw_features = self.test_data[data_timestamp] #（12，228，8）
            new_features = self.new_data[data_timestamp] #（12，228，8）

            # evaluate
            temp_predicts= inference(raw_features,timeStampModel, regression,)
            new_predicts = inference(new_features,timeStampModel, regression,)

            pred .append( temp_predicts.detach().tolist() )
            pred_new.append( new_predicts.detach().tolist() )

        print("===============================================")
        return pred,pred_new
...
if __name__ == '__main__':
    ...
    # # Example: Instantiate DataLoader for NYC congestion data
    # nyc_data_loader = DataLoader(config, 'NYC_congestion', pred_len)
    # nyc_train_data, nyc_train_label, nyc_test_data, nyc_test_label, nyc_adj = nyc_data_loader.load_data()
    # # Example: Instantiate TrafficModel for NYC congestion data
    # nyc_model = TrafficModel(nyc_train_data, nyc_train_label, nyc_test_data, nyc_test_label, nyc_adj,
    #                          config, 'NYC_congestion', input_size, out_size, GNN_layers, epochs, device,
    #                          num_timestamps, pred_len, save_flag, PATH, t_debug, b_debug)
    # # Example: Run the trained model on NYC congestion data
    # nyc_model.run_Trained_Model()
    # # Example: Access predictions from the trained model
    # # Note: This is just an example; adjust according to your model's output structure
    # predicted_values = nyc_model.predicted_values
    # ground_truth_values = nyc_model.ground_truth_values
